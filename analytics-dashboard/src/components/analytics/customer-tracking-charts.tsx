...
export function CustomerTrackingCharts({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <ChartContainer title="Segment BNRY" subtitle="Subscribers and average BNRY balance">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={segmentData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
            ...
            <Tooltip contentStyle={CHART_TOOLTIP_STYLE} />
            <Legend wrapperStyle={{ fontSize: "11px" }} />
            <Bar yAxisId="left" dataKey="count" name="Subscribers" fill="#FF6B00" radius={[4, 4, 0, 0]} />
            <Bar yAxisId="right" dataKey="avgLtv" name="Avg BNRY" fill="#10B981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </ChartContainer>

      <ChartContainer title="Acquisition Funnel" subtitle="Subscriber conversion stages (%)">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(_value, _name, item) => [
                `${formatNumber(Number(item.payload.count ?? 0), true)} (${item.payload.rate.toFixed(1)}%)`,
                "Count",
              ]}
            />
      ...
      <ChartContainer title="Acquisition Channels" subtitle="Sample channel mix & conversion">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={channelData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
            ...
            <Tooltip contentStyle={CHART_TOOLTIP_STYLE} />
            <Bar dataKey="count" name="Subscribers" fill="#FF6B00" radius={[4, 4, 0, 0]}>
              {channelData.map((_, i) => (
      ...
      <ChartContainer title="Earn Channels" subtitle="Channel scale & retention">
        <ResponsiveContainer width="100%" height={220}>
